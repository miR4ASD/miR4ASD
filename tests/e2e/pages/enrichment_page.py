...
class EnrichmentPage(BasePage):
    ...
    DB_BADGE_ID = "#enrichment-db-badge"
    ...
    def is_awaiting_selection_prompt_visible(self) -> bool:
        ...
        return "Select miRNAs to Analyze" in chips_box.inner_text()
    ...
    def get_selected_mirna_count(self) -> int:
        """Return the number of selected miRNAs shown in the enrichment miRNA field."""
        return int(self.page.locator("#enrichment-mirna-count").inner_text().strip())

    def get_selected_mirna_chips(self) -> list:
        """Return the list of selected-miRNA chip labels in the enrichment field."""
        return [
            c.strip()
            for c in self.page.locator(
                "#enrichment-mirna-chips .badge.bg-primary"
            ).all_inner_texts()
        ]

    def fill_enrichment_mirna_input(self, text: str) -> None:
        """Type miRNA IDs into the enrichment miRNA field textarea."""
        editor = self.page.locator("#enrichment-mirna-input")
        if not editor.is_visible():
            self.page.locator(
                "[data-bs-target='#enrichmentMirnaEditorCollapse']"
            ).click()
            self.page.wait_for_timeout(400)
        editor.fill(text)
        self.page.wait_for_timeout(700)

    def click_copy_enrichment_mirnas(self) -> None:
        """Click Copy in the enrichment miRNA field header."""
        self.page.locator("#btn-copy-enrichment-mirnas").click()
        self.page.wait_for_timeout(400)

    def click_clear_enrichment_mirnas(self) -> None:
        """Click Clear in the enrichment miRNA field header (clears the selection)."""
        self.page.locator("#btn-clear-enrichment-mirnas").click()
        self.page.wait_for_timeout(500)
