...
class StudiesTablePage(BasePage):
    ...
    def __init__(
        ...
        Args:
            ...
            show_selected_btn: Selector of the per-table "Selected only" toggle button.
        """
        ...
        self.show_selected_btn = show_selected_btn
    ...
    def toggle_show_selected_only(self) -> None:
        """Toggle this table's "Selected only" view (shows only checked miRNA rows)."""
        self.page.locator(self.show_selected_btn).click()
        self.page.wait_for_timeout(500)

    def is_show_selected_only_enabled(self) -> bool:
        """Return True when the "Selected only" toggle is enabled (selection exists)."""
        btn = self.page.locator(self.show_selected_btn)
        return not btn.is_disabled() and "disabled" not in (
            btn.get_attribute("class") or ""
        )

    def is_show_selected_only_active(self) -> bool:
        """Return True when the "Selected only" toggle is in its active state."""
        return "active" in (
            self.page.locator(self.show_selected_btn).get_attribute("class") or ""
        )
